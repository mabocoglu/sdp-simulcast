    ...
    if (simGroup) {
        ...
        return [mLine.ssrcs && mLine.ssrcs
            ...
            .filter(function(ssrc) {
                return !desktopSSRCList || !desktopSSRCList.includes(ssrc); // we should find non desktop ssrc
            })[0]];
    }
}
...
Simulcast.prototype._restoreSimulcast = function(mLine, desktopMSID) {
    ...
    if (numSsrcs === 1) {
        ...
        // there might be more than one fid groups.
...
findSsrcsForDesktopMsid = function(ssrcs, desktopMsid) {
    ...
    // TODO: We may use filter method here
    for (const ssrc of ssrcs) {

        if (ssrc.attribute === "msid" && ssrc.value === desktopMsid) {
            // adding desktop ssrc ids into the array.
